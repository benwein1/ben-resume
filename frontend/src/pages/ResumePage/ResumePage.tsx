...
import { getCandidate, updateCandidate } from "../../api/internal/candidates";
import { EditResumeDrawer } from "../../components/resume/EditResumeDrawer";
import type { ResumeData } from "../../types/resume";
import {
  jumpTo,
  mapCandidateToResume,
  sectionOrder,
  toCandidateUpdate,
  toUrl,
} from "./ResumePageUtils";

import "./ResumePage.scss";

export function ResumePage() {
  const benCandidateId = Number(import.meta.env.VITE_BEN_CANDIDATE_ID ?? 1);
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const candidate = await getCandidate(benCandidateId);
        setApiData(mapCandidateToResume(candidate));
      } catch (err) {
        setStatus((err as Error).message || "Failed to load resume data");
      }
    };
    void load();
  }, [benCandidateId, mapCandidateToResume]);
    ...
    <main className="resume-page">
      <section className="resume-hero">
        ...
        <div className="resume-hero__socials">
          <a href={toUrl(view.linkedIn)} target="_blank" rel="noreferrer">
            <LinkedInIcon fontSize="small" /> LinkedIn
          </a>
          <a href={toUrl(view.gitHub)} target="_blank" rel="noreferrer">
      ...
      {status ? <p className="resume-page__status">{status}</p> : null}
      ...
      <EditResumeDrawer
        ...
        onSave={async () => {
          const updated = await updateCandidate(
            benCandidateId,
            toCandidateUpdate(draft),
          );
          setApiData(mapCandidateToResume(updated));
          setStatus("Saved.");
          setDrawerOpen(false);
        }}
      />
